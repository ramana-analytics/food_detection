...
Grains = 0
Fruits = 0
groups = ['Proteins', 'Dairy', 'Grains', 'Fruits', 'Vegetables']

tempDF = FetchDataFromSelectedGroups(patientId, start_date, end_date)
...
for i in tempDF:
    for j in i.split(','):
        if j == "Proteins":
            ...
            continue
values = [Proteins, Dairy, Grains, Fruits, Vegetables]
print(values)
N = len(groups)
values += values[:1]
